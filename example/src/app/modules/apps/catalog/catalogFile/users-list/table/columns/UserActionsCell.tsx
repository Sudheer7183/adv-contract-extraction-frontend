...
mutation deleteCatalogfile($id: Int!){
  deleteCatalogfile(id: $id){
    ok
  }
}
`
...
const UserActionsCell: FC<Props> = ({ id }) => {
  console.log("catalog edit id-->", id);
  ...
  const openEditModal = () => {
    ...
    setIsEdit(true)
    nav("/catalog-management/catalog/editcatalog/" + id)
  }


  // const deletecatalog = (id: any) => {
  //   request(`${BASEURL}graphql/`, deleteCatalog, { id: id }).then((res: any) => {
  ...
  const handleDeleteConfirmed = async () => {
    try {
      const response = await request(`${BASEURL}graphql/`, deleteCatalog, { id: id }, {Authorization: `Bearer ${localStorage.getItem('Token')}`});
