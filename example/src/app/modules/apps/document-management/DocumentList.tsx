...
const DocumentList = () => {
    ...
    // console.log("fileProcess->",fileStatus.fileStatusPiechart.length);
                    ...
                    renderRowActions={({ row }) => (
                        <Box sx={{ display: 'flex', gap: '1rem' }}>
                            ...
                            {row.original.lock == true ?
                                <Button data-bs-toggle="tooltip-inverse" title='File Unlock' style={{ width: '10%' }}><LockPersonIcon className='file_unlock' onClick={() => handleLock(row.original.id)} /></Button>
                                : null}
                            {row.original.fileStatus == "To be Processed" ?
